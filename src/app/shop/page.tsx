...
function ShopContent() {
  ...
  // Exclude bidding items from Shop All
  const shopProducts = React.useMemo(() => products.filter(p => !p.isBiddingItem), [products])

  // Apply filters to non-bidding products
  const filteredProducts = applyFilters(shopProducts)
          ...
          <div className="flex-1">
            <div className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                ...
                <p className="text-gray-600">
                  <span className="font-semibold text-black">{shopProducts.length} total items</span>
                </p>
                {filters.searchQuery && (
            ...
            <div className="mb-4 flex justify-between items-center">
              <p className="text-gray-600">
                ...
                {filteredProducts.length !== shopProducts.length && (
                  <span className="text-black font-semibold"> (filtered from {shopProducts.length} total)</span>
                )}
              </p>
            </div>
