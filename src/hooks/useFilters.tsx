...
export function FilterProvider({ children }: { children: ReactNode }) {
  ...
  const applyFilters = (products: Product[]): Product[] => {
    const filteredProducts = products.filter(product => {
      ...
      // Category filter with Pants grouping
      if (filters.category.length > 0) {
        if (filters.category.includes('Pants')) {
          // Include both Pants and Collection when Pants is selected
          if (!['Pants', 'Collection'].includes(product.category)) {
            return false;
          }
        } else if (!filters.category.includes(product.category)) {
          return false;
        }
      }
